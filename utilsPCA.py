...
def generate_and_set_pca_broadcast_equal_int(current_feature, current_fmap, logger=None):
    ...
    def adjust_tensor(tensor, target_mean, target_std):
        ...
        epsilon = 1e-10
        adjusted_tensor = (tensor - current_mean) * (target_std / (current_std + epsilon)) + target_mean
    ...
    for i in range(0, current_feature.weight.shape[1]):
        ...
        for small_batch in temp:
            # only for this specific line, since sometimes the denominator is zero
            with np.errstate(divide='ignore', invalid='ignore'):
                try:
                    batch_to_pca(small_batch, pca_increment_object, filter_shape)
                except Exception as e:
                    print(f'there is an error here of {e}, so instead this iteration is skipped')
            # this was used for error checking, but everything looks fine. Sometimes a full tensor of zero appears, and that is normal since there was a RELU as the last fmap sometimes, and brings all the values to zero 
            # the solution to this is to just turn off the error messages I guess using np.seterr(divide='ignore')
            # if torch.all(small_batch==0):
            #     print(f'its all zeros')
            # if torch.isnan(small_batch).any():
            #     print(f'it has NaNs')
    ...
    # TODO MOVE THE RANDOM INT WEIGHTS OUT
    # this is the random int weight 
    temp_storage = temp1.detach()
    target_mean_A = temp1.mean().item()
    target_std_A = temp1.std().item()

    custom_print(f'comparing the original random int to the generated int')
    ...
    custom_print(f'ratio of ran int to pca is {target_mean_A/conv2_weight.mean().item()}')

    # matching mean and std div, and then pushing it to 10 times that of the 
    conv2_weight = torch.from_numpy(conv2_weight).to(torch.float).cuda()
    conv2_weight = adjust_tensor(
        tensor=conv2_weight, 
        target_mean=target_mean_A,
        target_std=target_std_A
    )       # PCA and random weights now have same mean and std div 

    conv2_weight = conv2_weight * 10        # make it 10 times that of normal 
    custom_print(f'ratio of ran int to adjusted pca is {target_mean_A/conv2_weight.mean().item()}')
    
    # adding ran int (1) to PCA (10)
    ...
    custom_print(f'If this is false, success -> {state_check == str(current_feature.state_dict())}')



def generate_and_set_pca_broadcast_equal_int_NO_SEED(current_feature, current_fmap, logger=None):
    """
    This is a modified method that broadcasts the weights instead. It should work as a drop in for the original method. This one additionally attempts to match the standard deviation and the mean of the weights so that the initialization does not go absolutely batshit crazy 

    2024 9 3
    this also additionally removes the seeding function so that we can see what would happen if we dont seed it. The hypothesis would be that the accuracy would drop quite drastically. This is part of experiment 3. Baseline establishment (BP on PCA)
    ...
    custom_print(f'comparing the original random int to the generated int')
    custom_print(f'NO SEEDING is done with this')
    custom_print(f'generated mean is {conv2_weight.mean().item()}, generated standard div is {conv2_weight.std().item()}')
    ...
    # adding ran int (1) to PCA (10)
    # final_weight = temp_storage + conv2_weight
    final_weight = conv2_weight         # so that this removes the seeding, which comes from temp_storage
